...
use plonk_verifier::{
    loader::evm::{encode_calldata, EvmYulLoader},
    pcs::kzg::{Gwc19, Kzg},
    system::halo2::{
        compile,
    ...
    verifier::{self, PlonkVerifier}, util::arithmetic::PrimeField,
};
...
fn compile_verifier(code: &str) -> Vec<u8> {
    ...
    for mat in re.find_iter(&output) {
        bytecode.push_str(mat.as_str());
    }
    hex_to_bytes(&bytecode).unwrap()
}
